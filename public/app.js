class GitHistoryUI {
            ...
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center space-x-2 mb-2">
                        <span class="text-sm font-mono text-gray-500 dark:text-gray-400">${commit.hash.substring(0, 8)}</span>
                        <span class="text-sm text-gray-600 dark:text-gray-400">${commit.author}</span>
                        <span class="text-sm text-gray-500 dark:text-gray-500">${this.formatDate(commit.date)}</span>
                    </div>
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">${commit.message}</h3>
                ...
                <div class="flex space-x-2">
                    <button class="px-3 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600" onclick="app.showCommitDetails('${commit.hash}')">
                        View
                    </button>
                </div>
            </div>
            ...
            details.innerHTML = `
                <div class="space-y-4">
                    <div class="border-b border-gray-200 dark:border-gray-700 pb-4">
                        <div class="flex items-center space-x-2 mb-2">
                            <span class="text-sm font-mono text-gray-500 dark:text-gray-400">${commit.hash}</span>
                            <span class="text-sm text-gray-600 dark:text-gray-400">by ${commit.author}</span>
                            <span class="text-sm text-gray-500 dark:text-gray-500">${this.formatDate(commit.date)}</span>
                        </div>
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white">${commit.message}</h3>
                    </div>
                    
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div>
                            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-3">Files Changed</h4>
                            <div class="space-y-2">
                                ${commit.files.map(file => `
                                    <div class="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded">
                                        <span class="text-sm text-gray-700 dark:text-gray-300">${file}</span>
                                        <button class="text-blue-500 hover:text-blue-600 text-sm" onclick="app.showFileDiff('${file}')">
                                            View Diff
                                        </button>
                                    </div>
                                `).join('')}
                        ...
                        <div>
                            <h4 class="text-lg font-medium text-gray-900 dark:text-white mb-3">Diff Summary</h4>
                            <div class="space-y-2">
                                ${diff.map(file => `
                                    <div class="p-2 bg-gray-50 dark:bg-gray-700 rounded">
                                        <div class="text-sm font-medium text-gray-700 dark:text-gray-300">${file.file}</div>
    ...
    switchView(view) {
        ...
        document.getElementById('graphView').className = view === 'graph' 
            ...
            : 'px-3 py-1 text-sm bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded hover:bg-gray-300 dark:hover:bg-gray-600';
